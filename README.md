...
```bash
python -m bedtime.library.seed        # index the 10-story library
...
give memory a warm start, and act as a regression net:
`python -m bedtime.library.seed --check` runs
...
Balance is deliberate — 4 girl protagonists, 2 boy, 1 sister-and-brother pair, 2
...
### What the calibration run actually says

I'd rather point at the uncomfortable result than let you find it.

The report checks whether the blended score beats both of its components at
ranking stories against my labels. On the current run it does **not**:

| | Spearman ρ |
|---|---|
| Blended score (what the gate uses) | **+0.640** |
| LLM rubric alone | +0.877 |
| Deterministic alone | −0.246 |

By the test the report itself states, that means the 75/25 weighting is wrong and
the rubric alone would rank better.

I have not re-tuned the weights on that result, and the reason is n. The golden
set is **10 stories, single-rater, labelled by me**. The 95% confidence interval
on ρ at n=10 is roughly ±0.4, so 0.640 and 0.877 are not distinguishable — they
are one interval sitting on top of another. Fitting weights to a gap that wide is
fitting to noise, and I would rather ship a defensible prior than an overfitted
constant. The deterministic term is in the blend because it is the only part of
the score that means the same thing after a model update, not because it wins a
correlation contest on ten stories.

What would settle it is more labels: ~100 stories, three independent raters,
inter-rater agreement reported before any correlation is. That is the first thing
I'd do with more time, and until it's done this number is a caution, not a
verdict.

Two other things the report flags and I have not acted on:

- It **recommends threshold 84**; the shipped default is 82. At 84 precision is
  1.00 and recall drops to 0.50 on ten stories — one story's worth of movement.
  82 keeps recall at 1.00 with precision 0.80. Neither is well-determined at this
  n, so I left the more forgiving one and wrote down why.
- The report is stamped `prompts v3.4.1`. Prompts are now `v3.5.0` (per-request
  story length). Re-run the three report scripts to refresh it — the version
  stamp exists
  precisely so this mismatch is visible rather than silent.
...
```bash
# tests - 277 of them, no API key and no network needed
pytest tests/ -q

# offline end-to-end story
python main.py --mock "a story about a shy dragon"

# the three reports (needs a key; roughly $0.40 for all three)
python -m bedtime.evaluation.calibrate
python -m bedtime.evaluation.run_eval
python -m bedtime.evaluation.red_team

# HTML monitoring view -> reports/dashboard.html
python -m bedtime.observability.dashboard

# the API on :8000
uvicorn bedtime.api:app --port 8000

# the web app
streamlit run app.py
```

The CI gate is four commands — secret scan, tests, the seed-library gate, and the
red-team suite with `--strict-exit` so the build fails on any successful attack:
...
- `.env` is gitignored; no key is committed. The CI gate above greps for one
  and fails the build if it finds it.
